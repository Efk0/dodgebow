...
public class SimpleConfig {
    ...
    public SimpleConfig(InputStream configStream, File configFile, int comments, JavaPlugin plugin) {
        ...
        try {
            newConfigStream = new InputStreamReader(configStream, "UTF8");
        } catch (UnsupportedEncodingException ex) {
            ex.printStackTrace();
            return;
        }
        this.config = YamlConfiguration.loadConfiguration(newConfigStream);
    }
    ...
    public void setIfNot(String path, Object value) {
        if(!this.config.contains(path))
            this.config.set(path, value);
    }

    public void setIfNot(String path, Object value, String comment) {
        if(!this.config.contains(path)) {
            this.config.set(manager.getPluginName() + "_COMMENT_" + comments, " " + comment);
            comments++;
    ...
    public void reloadConfig() {
        Reader fileReader;
        try {
            fileReader = new InputStreamReader(manager.getConfigContent(file), "UTF8");
        } catch (UnsupportedEncodingException ex) {
            ex.printStackTrace();
            return;
        }
        this.config = YamlConfiguration.loadConfiguration(fileReader);
    }
